...
class _DTM4PViewProvider:
    ...
    def _open_edit_panel(self, obj):
        ...
        if _frame_is_set(obj):
            initial_frame = (
                Vector(obj.DTM4P_FrameAnchor),
                Vector(obj.DTM4P_FrameU),
                Vector(obj.DTM4P_FrameV),
                Vector(obj.DTM4P_FrameNormal),
            )
...
def _ensure_frame_properties(obj):
    ...
    """
    if not hasattr(obj, "DTM4P_FrameAnchor"):
        obj.addProperty(
            "App::PropertyVector", "DTM4P_FrameAnchor", "DTM4P",
            "Frame anchor point in world coordinates")
    if not hasattr(obj, "DTM4P_FrameU"):
        obj.addProperty(
            "App::PropertyVector", "DTM4P_FrameU", "DTM4P",
            "Frame U axis (housing local X)")
    if not hasattr(obj, "DTM4P_FrameV"):
        obj.addProperty(
            "App::PropertyVector", "DTM4P_FrameV", "DTM4P",
            "Frame V axis (housing local Y)")
    if not hasattr(obj, "DTM4P_FrameNormal"):
        obj.addProperty(
            "App::PropertyVector", "DTM4P_FrameNormal", "DTM4P",
            "Frame normal (housing local Z, outward from face)")


def _frame_is_set(obj):
    """Return True if the stored frame on *obj* looks valid."""
    n = getattr(obj, "DTM4P_FrameNormal", None)
    return n is not None and n.Length > 1e-9


def _store_frame(obj, anchor, u_axis, v_axis, normal):
    _ensure_frame_properties(obj)
    obj.DTM4P_FrameAnchor = Vector(anchor)
    obj.DTM4P_FrameU = Vector(u_axis)
    obj.DTM4P_FrameV = Vector(v_axis)
    obj.DTM4P_FrameNormal = Vector(normal)
...
def _resolve_frame(obj, base_shape):
    ...
    """
    from dtm4p_geometry import compute_face_frame, compute_edge_frame
    ...
    face_name = getattr(obj, "DTM4P_FaceName", "")
    if not face_name:
    ...
    if face_idx < 1 or face_idx > len(base_shape.Faces):
        FreeCAD.Console.PrintWarning(
            "DTM4P: stored {} no longer resolves on base shape "
    ...
    FreeCAD.Console.PrintMessage(
        "DTM4P: migrated {} frame to absolute coordinates.\n"
        .format(obj.Name))
    return anchor, u, v, n
...
class DTM4PTaskPanel:
    ...
    def accept(self):
        from dtm4p_geometry import place_housing_from_frame
        ...
        try:
            ...
            FreeCAD.Console.PrintError(
                "DTM4P frame computation failed: {}\n".format(e))
