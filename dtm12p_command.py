"""FreeCAD command and task-panel UI for the DTM12P Termination Housing tool."""
...
class _DTM12PProxy:
    """Data proxy for a DTM12P PartDesign::FeatureAdditivePython."""
    ...
    def execute(self, obj):
        ...
        FreeCAD.Console.PrintMessage(
            "DTM12P.execute() called for {}\n".format(obj.Name))
        if not hasattr(obj, "DTM12P_FaceName"):
            FreeCAD.Console.PrintWarning(
                "DTM12P.execute(): no DTM12P_FaceName property yet\n")
            return
        ...
        elif hasattr(obj, "DTM12P_OriginalBody") and obj.DTM12P_OriginalBody:
            original = FreeCAD.ActiveDocument.getObject(
                obj.DTM12P_OriginalBody)
        ...
        try:
            ...
            new_shape = place_housing_from_frame(
                ...
                x_offset=getattr(obj, "DTM12P_XOffset", 0.0),
                y_offset=getattr(obj, "DTM12P_YOffset", 0.0),
                rotation=getattr(obj, "DTM12P_Rotation", 0.0),
            )
...
class _DTM12PViewProvider:
    """ViewProvider that enables double-click re-editing of DTM12P objects."""
    ...
    def _open_edit_panel(self, obj):
        ...
        edge_name = getattr(obj, "DTM12P_EdgeName", "")
        initial_frame = None
        _ensure_frame_properties(obj)
        if _frame_is_set(obj):
            initial_frame = (
                Vector(obj.DTM12P_FrameAnchor),
                Vector(obj.DTM12P_FrameU),
                Vector(obj.DTM12P_FrameV),
                Vector(obj.DTM12P_FrameNormal),
            )
        panel = DTM12PTaskPanel(
            original, obj.DTM12P_FaceName,
            edge_name=edge_name,
    ...
    def getIcon(self):
        return os.path.join(
            _plugin_dir, "resources", "icons", "DTM12P.svg"
        )
...
def _add_dtm12p_properties(obj):
    """Add custom storage properties to a DTM12P feature object."""
    obj.addProperty(
        "App::PropertyFloat", "DTM12P_XOffset", "DTM12P",
        "Offset along the face U axis (mm)")
    obj.addProperty(
        "App::PropertyFloat", "DTM12P_YOffset", "DTM12P",
        "Offset along the face V axis (mm)")
    obj.addProperty(
        "App::PropertyFloat", "DTM12P_Rotation", "DTM12P",
        "Rotation angle on the face (degrees)")
    obj.addProperty(
        "App::PropertyString", "DTM12P_FaceName", "DTM12P",
        "Face used on the original body (display only)")
    obj.addProperty(
        "App::PropertyString", "DTM12P_EdgeName", "DTM12P",
        "Edge used for front alignment (display only)")
    obj.addProperty(
        "App::PropertyString", "DTM12P_OriginalBody", "DTM12P",
...
def _ensure_frame_properties(obj):
    ...
    """
    if not hasattr(obj, "DTM12P_FrameAnchor"):
        obj.addProperty(
            "App::PropertyVector", "DTM12P_FrameAnchor", "DTM12P",
            "Frame anchor point in world coordinates")
    if not hasattr(obj, "DTM12P_FrameU"):
        obj.addProperty(
            "App::PropertyVector", "DTM12P_FrameU", "DTM12P",
            "Frame U axis (housing local X)")
    if not hasattr(obj, "DTM12P_FrameV"):
        obj.addProperty(
            "App::PropertyVector", "DTM12P_FrameV", "DTM12P",
            "Frame V axis (housing local Y)")
    if not hasattr(obj, "DTM12P_FrameNormal"):
        obj.addProperty(
            "App::PropertyVector", "DTM12P_FrameNormal", "DTM12P",
            "Frame normal (housing local Z, outward from face)")


def _frame_is_set(obj):
    """Return True if the stored frame on *obj* looks valid."""
    n = getattr(obj, "DTM12P_FrameNormal", None)
    return n is not None and n.Length > 1e-9


def _store_frame(obj, anchor, u_axis, v_axis, normal):
    _ensure_frame_properties(obj)
    obj.DTM12P_FrameAnchor = Vector(anchor)
    obj.DTM12P_FrameU = Vector(u_axis)
    obj.DTM12P_FrameV = Vector(v_axis)
    obj.DTM12P_FrameNormal = Vector(normal)
...
def _resolve_frame(obj, base_shape):
    ...
    """
    from dtm12p_geometry import compute_face_frame, compute_edge_frame
    ...
    face_name = getattr(obj, "DTM12P_FaceName", "")
    if not face_name:
    ...
    if face_idx < 1 or face_idx > len(base_shape.Faces):
        FreeCAD.Console.PrintWarning(
            "DTM12P: stored {} no longer resolves on base shape "
    ...
    FreeCAD.Console.PrintMessage(
        "DTM12P: migrated {} frame to absolute coordinates.\n"
        .format(obj.Name))
    return anchor, u, v, n


def _get_dtm12p_base_object(obj):
    """Return the base object that a DTM12P result was derived from."""
    if hasattr(obj, "BaseFeature") and obj.BaseFeature is not None:
        return obj.BaseFeature
    orig_name = getattr(obj, "DTM12P_OriginalBody", None)
    if orig_name:
...
class DTM12PTaskPanel:
    """Task panel shown in the FreeCAD sidebar when the DTM12P command is active."""
    ...
    def _build_ui(self):
        ...
        header = QtWidgets.QLabel("<b>DTM12P Termination Housing</b>")
        header.setAlignment(QtCore.Qt.AlignCenter)
        layout.addRow(header)

        info = QtWidgets.QLabel(
            "<i>Places a Deutsch DTM04-12PA termination housing\n"
    ...
    def accept(self):
        from dtm12p_geometry import place_housing_from_frame
        ...
        try:
            ...
            FreeCAD.Console.PrintError(
                "DTM12P frame computation failed: {}\n".format(e))
        ...
        if is_pd_edit:
            ...
            result_obj.DTM12P_XOffset = x_offset
            result_obj.DTM12P_YOffset = y_offset
            result_obj.DTM12P_Rotation = rotation
            result_obj.DTM12P_FaceName = self.face_name
            result_obj.DTM12P_EdgeName = self.edge_name
            _store_frame(result_obj, anchor, u_axis, v_axis, normal)
        elif body is not None:
            result_obj = doc.addObject(
                "PartDesign::FeatureAdditivePython", "DTM12P")
            _DTM12PProxy(result_obj)
            _DTM12PViewProvider(result_obj.ViewObject)
            _add_dtm12p_properties(result_obj)
            result_obj.DTM12P_XOffset = x_offset
            result_obj.DTM12P_YOffset = y_offset
            result_obj.DTM12P_Rotation = rotation
            result_obj.DTM12P_FaceName = self.face_name
            result_obj.DTM12P_EdgeName = self.edge_name
            result_obj.DTM12P_OriginalBody = body.Name
            _store_frame(result_obj, anchor, u_axis, v_axis, normal)
            body.addObject(result_obj)
        else:
            if self.edit_obj is not None:
                doc.removeObject(self.edit_obj.Name)
            result_obj = doc.addObject("Part::FeaturePython", "DTM12P")
            _DTM12PProxy(result_obj)
            _DTM12PViewProvider(result_obj.ViewObject)
            _add_dtm12p_properties(result_obj)
            result_obj.DTM12P_XOffset = x_offset
            ...
            result_obj.DTM12P_FaceName = self.face_name
            result_obj.DTM12P_EdgeName = self.edge_name
            result_obj.DTM12P_OriginalBody = self.body_obj.Name
            _store_frame(result_obj, anchor, u_axis, v_axis, normal)
...
class DTM12PCommand:
    """FreeCAD command that places a DTM12P termination housing onto a face."""
    ...
    def GetResources(self):
        return {
            ...
            "MenuText": "Place DTM12P Housing",
            "ToolTip": (
                "DTM12P\n"
                "Place a Deutsch DTM04-12PA termination housing onto the\n"
                "selected flat face, extending outward from the surface.\n"
                "Select a Face + Edge to align the housing front to the edge.\n"
                "Select an existing DTM12P result to re-edit."
            ),
        }
    ...
    def Activated(self):
        ...
        if not is_planar:
            QtWidgets.QMessageBox.warning(
                ...
                "Non-Planar Face",
                "Please select a flat (planar) face.\n"
                "The DTM12P housing can only be placed on flat surfaces.",
            )
            return

        panel = DTM12PTaskPanel(obj, face_name, edge_name=edge_name)
        FreeCADGui.Control.showDialog(panel)


FreeCADGui.addCommand("TrailCurrent_DTM12P", DTM12PCommand())
